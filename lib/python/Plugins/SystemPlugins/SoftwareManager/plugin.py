...
config.plugins.softwaremanager.updatetype = ConfigSelection([('hot', _('Upgrade with GUI')), ('cold', _('Unattended upgrade without GUI'))], 'hot')
...
class UpdatePluginMenu(Screen):
    ...
    def __init__(self, session, args = 0):
        ...
        if self.menu == 0:
            ...
            self.list.append(('install-extensions',
             _('Manage extensions'),
             _('\nManage extensions or plugins for your %s %s') % (getMachineBrand(), getMachineName()) + self.oktext,
             None))
            self.list.append(('software-update',
             _('Software update'),
             _('\nOnline update of your %s %s software.') % (getMachineBrand(), getMachineName()) + self.oktext,
             None))
            self.list.append(('software-restore',
             _('Software restore'),
             _('\nRestore your %s %s with a new firmware.') % (getMachineBrand(), getMachineName()) + self.oktext,
             None))
            if not boxtype.startswith('az') and not boxtype.startswith('dm') and not brandoem.startswith('cube'):
                self.list.append(('flash-online',
                 _('Flash Online'),
                 _('\nFlash on the fly your %s %s.') % (getMachineBrand(), getMachineName()) + self.oktext,
                 None))
            if not boxtype.startswith('az') and not brandoem.startswith('cube'):
                self.list.append(('backup-image',
                 _('Backup Image'),
                 _('\nBackup your running %s %s image to HDD or USB.') % (getMachineBrand(), getMachineName()) + self.oktext,
                 None))
            ...
            self.list.append(('system-restore',
             ...
             _('\nRestore your %s %s settings.') % (getMachineBrand(), getMachineName()) + self.oktext,
             None))
            self.list.append(('ipkg-install',
             _('Install local extension'),
             _('\nScan for local extensions and install them.') + self.oktext,
             None))
            for p in plugins.getPlugins(PluginDescriptor.WHERE_SOFTWAREMANAGER):
                if p.__call__.has_key('SoftwareSupported'):
                    ...
                    if callFnc is not None:
                        ...
                        self.list.append(('default-plugin',
                         menuEntryName,
                         menuEntryDescription + self.oktext,
                         callFnc))

            if config.usage.setup_level.index >= 2:
                self.list.append(('advanced',
                 _('Advanced options'),
            ...
            self.list.append(('backupfiles',
             _('Show default backup files'),
             _('Here you can browse (but not modify) the files that are added to the backupfile by default (E2-setup, channels, network).') + self.oktext + '\n\n' + self.infotext,
             None))
            self.list.append(('backupfiles_addon',
             _('Select additional backup files'),
             _('Here you can specify additional files that should be added to the backup file.') + self.oktext + '\n\n' + self.infotext,
             None))
            self.list.append(('backupfiles_exclude',
             _('Select excluded backup files'),
             _('Here you can select which files should be excluded from the backup.') + self.oktext + '\n\n' + self.infotext,
             None))
            if config.usage.setup_level.index >= 2:
                self.list.append(('ipkg-manager',
                 _('Packet management'),
                 _('\nView, install and remove available or installed packages.') + self.oktext,
                 None))
            self.list.append(('ipkg-source',
             _('Select upgrade source'),
            ...
            for p in plugins.getPlugins(PluginDescriptor.WHERE_SOFTWAREMANAGER):
                if p.__call__.has_key('AdvancedSoftwareSupported'):
                    ...
                    if callFnc is not None:
                        ...
                        self.list.append(('advanced-plugin',
                         menuEntryName,
                         menuEntryDescription + self.oktext,
                         callFnc))
...
class SoftwareManagerInfo(Screen):
    ...
    def __init__(self, session, skin_path = None, mode = None, submode = None):
        ...
        if self.skin_path == None:
            self.skin_path = resolveFilename(SCOPE_CURRENT_PLUGIN, 'SystemPlugins/SoftwareManager')
        self['actions'] = ActionMap(['ShortcutActions', 'WizardActions'], {'back': self.close,
         'red': self.close}, -2)
        self.list = []
...
class PluginManager(Screen, PackageInfoHandler):
    ...
    def setState(self, status = None):
        if status:
            ...
            if status == 'update':
                ...
                self.statuslist.append((_('Updating software catalog'),
                 '',
                 _('Searching for available updates. Please wait...'),
                 '',
                 '',
                 statuspng,
                 divpng,
                ...
                self.statuslist.append((_('Package list update'),
                 '',
                 _('Searching for new installed or removed packages. Please wait...'),
                 '',
                 '',
                 statuspng,
                 divpng,
                ...
                self.statuslist.append((_('Error'),
                 '',
                 _('An error occurred while downloading the packetlist. Please try again.'),
                 '',
                 '',
                 statuspng,
                 divpng,
                        ...
                        if categorytag == foundtag:
                            ...
                            if attributes.has_key('packagetype'):
                                if attributes['packagetype'] == 'internal':
                                    continue
                                self.packetlist.append([attributes['name'],
                                 attributes['details'],
    ...
    def buildCategoryComponent(self, tag = None):
        ...
        if tag is not None:
            if tag == 'System':
                return (_('System'),
                 _('View list of available system extensions'),
                 tag,
                 divpng)
            elif tag == 'Skin':
                return (_('Skins'),
                 _('View list of available skins'),
                 tag,
                 divpng)
            elif tag == 'Recording':
                return (_('Recordings'),
                 _('View list of available recording extensions'),
                 tag,
                 divpng)
            elif tag == 'Network':
                return (_('Network'),
                 _('View list of available networking extensions'),
                 tag,
                 divpng)
            elif tag == 'CI':
                return (_('Common Interface'),
                 _('View list of available CommonInterface extensions'),
                 tag,
                 divpng)
            elif tag == 'Default':
                return (_('Default settings'),
                 _('View list of available default settings'),
                 tag,
                 divpng)
            elif tag == 'SAT':
                return (_('Satellite equipment'),
                 _('View list of available Satellite equipment extensions.'),
                 tag,
                 divpng)
            elif tag == 'Software':
                return (_('Software'),
                 _('View list of available software extensions'),
                 tag,
                 divpng)
            elif tag == 'Multimedia':
                return (_('Multimedia'),
                 _('View list of available multimedia extensions.'),
                 tag,
                 divpng)
            elif tag == 'Display':
                return (_('Display and user interface'),
                 _('View list of available display and userinterface extensions.'),
                 tag,
                 divpng)
            elif tag == 'EPG':
                return (_('Electronic Program Guide'),
                 _('View list of available EPG extensions.'),
                 tag,
                 divpng)
            elif tag == 'Communication':
                return (_('Communication'),
                 _('View list of available communication extensions.'),
                 tag,
                 divpng)
            else:
                return (str(tag),
                 _('View list of available ') + str(tag) + ' ' + _('extensions.'),
                 tag,
                 divpng)
        return
...
class PluginManagerInfo(Screen):
    ...
    def __init__(self, session, plugin_path, cmdlist = None):
        ...
        self['shortcuts'] = ActionMap(['ShortcutActions', 'WizardActions'], {'ok': self.process_all,
         'back': self.exit,
         'red': self.exit,
         'green': self.process_extensions}, -1)
        self.list = []
        self['list'] = List(self.list)
        self['key_red'] = StaticText(_('Cancel'))
...
class PluginManagerHelp(Screen):
    ...
    def __init__(self, session, plugin_path):
        ...
        self.session = session
        self.skin_path = plugin_path
        self['shortcuts'] = ActionMap(['ShortcutActions', 'WizardActions'], {'back': self.exit,
         'red': self.exit}, -1)
        self.list = []
        self['list'] = List(self.list)
        self['key_red'] = StaticText(_('Close'))
    ...
    def buildEntryComponent(self, state):
        ...
        installpng = LoadPixmap(cached=True, path=resolveFilename(SCOPE_CURRENT_PLUGIN, 'SystemPlugins/SoftwareManager/install.png'))
        if state == 'installed':
            return (_('This plugin is installed.'),
             _('You can remove this plugin.'),
             installedpng,
             divpng)
        if state == 'installable':
            return (_('This plugin is not installed.'),
             _('You can install this plugin.'),
             installablepng,
             divpng)
        if state == 'install':
            return (_('This plugin will be installed.'),
             _('You can cancel the installation.'),
             installpng,
             divpng)
        if state == 'remove':
            return (_('This plugin will be removed.'),
             _('You can cancel the removal.'),
             removepng,
             divpng)
...
class PluginDetails(Screen, PackageInfoHandler):
    ...
    def setThumbnail(self, noScreenshot = False):
        ...
        self.picload.setPara((self['screenshot'].instance.size().width(),
         self['screenshot'].instance.size().height(),
         sc[0],
         sc[1],
         False,
         1,
    ...
    def go(self):
        ...
        elif iSoftwareTools.NetworkConnectionAvailable:
            if self.packagefiles:
                for package in self.packagefiles[:]:
                    self.cmdList.append((IpkgComponent.CMD_INSTALL, {'package': package['name']}))
                    if len(self.cmdList):
...
class PacketManager(Screen, NumericalTextInput):
    ...
    def __init__(self, session, plugin_path, args = None):
        ...
        self.cache_file = eEnv.resolve('${libdir}/enigma2/python/Plugins/SystemPlugins/SoftwareManager/packetmanager.cache')
        self.oktext = _('\nAfter pressing OK, please wait!')
...
class ShowUpdatePackages(Screen, NumericalTextInput):
    ...
    def __init__(self, session, plugin_path, args = None):
        ...
        self['shortcuts'] = NumberActionMap(['ShortcutActions',
         ...
         'InputActions',
         'InputAsciiActions',
         'KeyboardInputActions'], {'back': self.exit,
         'red': self.exit,
         'ok': self.exit,
...
def Plugins(path, **kwargs):
    global plugin_path
    plugin_path = path
    list = [PluginDescriptor(name=_('Software management'), description=_("Manage your %s %s's software") % (getMachineBrand(), getMachineName()), where=PluginDescriptor.WHERE_MENU, needsRestart=False, fnc=startSetup), PluginDescriptor(name=_('Ipkg'), where=PluginDescriptor.WHERE_FILESCAN, needsRestart=False, fnc=filescan)]
    if config.usage.setup_level.index >= 2:
